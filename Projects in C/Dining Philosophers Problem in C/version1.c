...
int numPhil; //total number of philosophers, processes
...
void eat(int tid){
    pthread_mutex_lock((tid +1) %numPhil == 0 ? &forks[0] :  &forks[tid]);
    pthread_mutex_lock((tid +1) %numPhil == 0 ? &forks[tid] : &forks[(tid+1)%numPhil] );
